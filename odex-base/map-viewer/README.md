...
This application integrate a Map view for Odex Open Suite.
...
- First login into Local server i.e. http://localhost:8080/odex-erp (admin/admin)
- Change 'proxy' server's url and suburl in '.env' file
  - Set `VITE_PROXY_TARGET` to http://localhost:8080
  - Set `VITE_PROXY_CONTEXT` to /odex-erp/
  - Note: If the server doesn't have a suburl, set `VITE_PROXY_CONTEXT` to `/`
- Try to run the app with a relative suburl i.e. on http://localhost:5173/odex-erp
